...
// rewrite vga driver for scrolling (some bugs)
...
void terminal_scroll(void){
	...
	for (size_t y = 2; y < VGA_HEIGHT; y++) {
		for (size_t x = 0; x < VGA_WIDTH; x++) {
			const size_t index = y * VGA_WIDTH + x;
			const size_t index_newline = (y - 2) * VGA_WIDTH + x;
			terminal_buffer[index_newline] = terminal_buffer[index];
			if(y == (VGA_HEIGHT - 1)){
				terminal_buffer[index] = vga_entry(' ', terminal_color);
				terminal_buffer[(y - 1) * VGA_WIDTH + x] = vga_entry(' ', terminal_color);
			}
		}
	}
}
